...
class SSL(Experiment):
    ...
    def set_conditions(self):
        ...
        self.conditions = [
            {"task": x[0], "pop_size": x[1]}
            for x in product(n_bandit_tasks, population_sizes)
        ]

    def create_network(self, condition):
        """Return a new network."""
        return self.models.SSLBanditNetwork(
            max_size=self.num_participants, condition=condition
        )
        ...
        if not self.networks():
            ...
            for condition in self.conditions:
                ...
                self.models.BanditParamsSource(
                    network=network, cond_num=condition["task"]
                )
